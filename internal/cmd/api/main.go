...
import (
	...
	flightapi "github.com/niiniyare/awo/internal"
...
func main() {
	...
	)
	{
		...
		bookingSvc = flightapi.NewBooking(logger)
	}
	...
	case "production":
		{
			...
			handleHTTPServer(ctx, u, flightEndpoints, bookingEndpoints, &wg, errc, logger, *dbgF)
		}

	case "development":
		{
			addr := "http://localhost:80/calc"
			u, err := url.Parse(addr)
			if err != nil {
				logger.Fatalf("invalid URL %#v: %s\n", addr, err)
			}
	...
	default:
		logger.Fatalf("invalid host argument: %q (valid hosts: production|development)\n", *hostF)
	}
