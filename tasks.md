...
- [ ] **STATUS: confirmed still open, deliberately not touched this pass** — separately tracked P0, out of scope for the security-boundary work done this pass.
- **GOAL:** `WorkflowTrigger` starts survive a crash or transient Temporal outage between commit and dispatch, as ADR-007 requires.
- **WHY:** `EntityService.startWorkflows` calls Temporal directly and synchronously; failure is a log line and a `// TODO`; no `workflow_outbox` table exists.
- **NEW FINDING (background/Temporal security-context audit, this pass):** `events/outbox/relay.go`'s `Relay.deliver` never restores a `tenant.TenantContext` from the event's own `TenantID` column before calling `Subscriber.HandleEvent` — it passes through the relay loop's own root context unchanged. If any subscriber calls repository code, `contrib/pgx.Repository.WithTx` would find no tenant context and silently skip `set_tenant_context` (mitigated by RLS at the SQL layer — see 1.10 — but still wrong at the Go layer). **Currently unreachable**: zero `events.Subscriber` implementations and zero `.Subscribe(...)` calls exist anywhere outside tests, no `events_outbox` migration exists, and the relay's own startup is commented out in `cmd/awo/serve_impl.go` ("outbox relay disabled until events_outbox migration is applied"). Whoever builds the outbox for real (this item) MUST restore `tenant.WithContext(ctx, tenant.TenantContext{TenantID: e.TenantID})` in `deliver` before calling any subscriber — do not carry the relay-loop context through unchanged.
- **FILES:** `api/service/entity.go` (`startWorkflows`), `events/outbox/relay.go` (`deliver` — tenant-context restoration, see above), new: a `workflow_outbox` table/migration, a relay worker.
- **IMPLEMENTATION:** Write the workflow start to a durable `workflow_outbox` row in the same TX as the entity mutation; a background relay (mirroring `events/outbox`'s relay, once its own tenant-context gap above is fixed) dispatches pending rows to Temporal via `workflow.WorkflowExecutor`, with WorkflowID-based dedup so retries are safe.
- **TESTS:** kill the process (or mock a Temporal-unavailable error) between commit and dispatch → confirm the workflow eventually starts once the relay runs. Confirm no duplicate workflow starts on relay retry (WorkflowID dedup). A test proving a dispatched event's handler sees the correct restored tenant context, not the relay loop's own.
- **ACCEPTANCE CRITERIA:** [ ] `workflow_outbox` table exists and is populated transactionally with the entity mutation. [ ] Relay worker dispatches pending rows. [ ] Crash-recovery test passes. [ ] `EntityService.startWorkflows` no longer calls Temporal directly. [ ] `events/outbox/relay.go`'s `deliver` restores tenant context before invoking a subscriber. [ ] `docs/08-workflow/OUTBOX_SPEC.md`/`TEMPORAL_INTEGRATION.md` match reality.
